...
namespace Valkyrie.Editor
{
    public static class SimulationCompiler
    {
        ...
        static void CreateSimulationServiceScript(string resultFilePath, List<Type> components, List<Type> systems)
        {
            ...
            fb.StartClass($"EcsSimulationService", "UnityEngine.MonoBehaviour");
            fb.Write(
                $"private readonly {typeof(GameSimulation).FullName} _simulationSystems = new {typeof(GameSimulation).FullName}();");
            fb.Write();
            ...
            fb.Write($"var dt = UnityEngine.Time.deltaTime;");
